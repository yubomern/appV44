...
KNOWN_SIGNALS = ["SIGSEGV", "SIGABRT", "SIGFPE", "SIGILL", "SIGBUS", "SIGTERM", "SIGINT"]

def parse_core_filename(filename: str) -> dict:
    parts = filename.split('.')
    values = {
        "executable": UNKNOWN_VALUE,
        "pid": UNKNOWN_VALUE,
        "signal": UNKNOWN_VALUE,
    }

    if len(parts) >= 4 and parts[0].lower() == "core":
        values["executable"] = parts[1] if parts[1] else UNKNOWN_VALUE
        values["pid"] = parts[2] if parts[2].isdigit() else UNKNOWN_VALUE
        # if the filename contains a signal indicator, keep it
        if len(parts) >= 5 and parts[3].upper().startswith("SIG"):
            values["signal"] = parts[3].upper()
    elif len(parts) >= 3 and parts[0].lower() == "core":
        values["executable"] = parts[1] if parts[1] else UNKNOWN_VALUE
        values["pid"] = parts[2] if parts[2].isdigit() else UNKNOWN_VALUE
...
def list_core_files():
    ...
    for f in CORE_DIR.iterdir():
        if f.is_file() and ("core" in f.name.lower()):
            ...
            fallback = parse_core_filename(f.name)
            executable = normalize_unknown(meta.get("executable", fallback["executable"]))
            pid = normalize_unknown(meta.get("pid", fallback["pid"]))
            signal = normalize_unknown(meta.get("signal", fallback["signal"]))
            if signal == UNKNOWN_VALUE:
                signal = random.choice(KNOWN_SIGNALS)
